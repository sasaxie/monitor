...
func StartHttpMonitor() {
	...
	ticker := time.NewTicker(
		time.Duration(config.MonitorConfig.Task.GetHTTPDataInterval) *
			time.Second)
	defer ticker.Stop()
	...
	for {
		...
		case <-ticker.C:
			for _, address := range models.NodeList.Addresses {
				go dealHttpMonitor(address)
			}
		}
	}
}

func dealHttpMonitor(address *models.Address) {
	...
	nodeInfoDetailTags := map[string]string{
		config.InfluxDBTagNode: addr,
		config.InfluxDBTagType: address.Type,
	...
	nodeInfoDetailFields := map[string]interface{}{
		...
		config.InfluxDBFieldTag:  address.Tag,

		config.InfluxDBFieldActiveConnectCount: nodeInfoDetail.ActiveConnectCount,
		...
		config.InfluxDBFieldDbVersion:                nodeInfoDetail.ConfigNodeInfo.DbVersion,
		...
		config.InfluxDBFieldCpuRate:             nodeInfoDetail.MachineInfo.CpuRate * 100,
		...
		config.InfluxDBFieldProcessCpuRate: nodeInfoDetail.MachineInfo.ProcessCpuRate * 100,
	...
	influxdb.Client.WriteByTime(
		config.InfluxDBPointNameNodeInfoDetail,
		nodeInfoDetailTags,
	...
	for _, p := range nodeInfoDetail.PeerList {
		...
		influxdb.Client.WriteByTime(
			config.InfluxDBPointNamePeerInfo,
			t,
			f,
			timeNow)
	}
